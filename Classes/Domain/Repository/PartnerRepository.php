...
class PartnerRepository extends Repository
{
     ...
     * that is a marker off the coast of Africa (ACE-709).
     *
     ...
     * writes: `GeocodeCommand` persists a `float` cast to string, which yields `"0"`.
     * A hand-entered `0.0` is not caught here and is dropped by the frontend module
     * instead, which parses the value. The two rules are therefore asymmetric on
     * purpose: a numeric comparison cannot be expressed portably while the column is
     * `VARCHAR` - Extbase's QOM has no cast, `column + 0` is a type error on
     * PostgreSQL, and `CAST` names its target type differently per platform. Changing
     * the column to a numeric type would remove the asymmetry; on the 3.x line that is
     * tracked as ACE-566, and neither branch has done it.
     *
