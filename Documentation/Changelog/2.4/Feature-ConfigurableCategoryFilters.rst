...
====================================================
Feature: Choose, order and label the partner filters
====================================================
...
Map` offered one select for every category type of the group `partners` that had
a category: all of them at once, in the order the types are registered in, and
each with the same "All options" label. Anything else needed an override of the
partial :file:`Partner/DemandCategories.html`.
...
..  list-table::
    ...
    *   -   :typoscript:`plugin.tx_academicpartners.filter.hideDisabledOptions`
        -   0
        -   Leaves out a category none of the listed partners carries,
...
:xml:`sys_category.partners.allOptions.<type>`, and falls back to
:xml:`sys_category.partners.allOptions` where there is none. The extension ships
no label per type; a site adds them in TypoScript:

..  code-block:: typoscript

    plugin.tx_academicpartners._LOCAL_LANG.default.sys_category.partners.allOptions.region = All regions
...
from another path, see :ref:`important-1790383005`.

The partial :file:`Partner/DemandCategories.html` loops the new variables
...
:php:`listAction()` or :php:`mapAction()`, or a template that renders the
partial with arguments of its own instead of :html:`{_all}` — it offers every
type with a category, as before, and the filter types and the visible count have
no effect there. To use them, let the overriding action call the parent action,
and pass :html:`filterTypes` on to the partial.

A project that overrides :file:`Partner/DemandCategories.html` keeps its own
