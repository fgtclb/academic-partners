..  _breaking-partners-images-render-as-picture:
...
The partner list item, the partnership list and teaser items and the partner
...
first `media` file of the partner page, which is the partner logo, and `logo`
applies no crop, so a wide logo is not cut into a card ratio. The page
template asks for `detail`.
...
`1` it uses already, and the page object of the partner page type with the
key `-1758484801`, below every theme and project path. The page object
...
*   A project that replaces the partial root paths of the plugin completely,
    or a page object that renders one of the item partials or the page template
    in a view of its own, fails with an exception on the partial
    `Academic/Image` that the view cannot resolve.

Affected Installations
======================

Every installation that renders a partner list, a partnership list or teaser,
or a page of the partner page type with an image.

Migration
=========

#.  Adjust CSS that addresses the partner image.
#.  A plugin view whose partial root paths were replaced needs the path of
    `EXT:academic_base` below the others:

    ..  code-block:: typoscript

        plugin.tx_academicpartners.view.partialRootPaths {
...
..  index:: Fluid, Frontend, TypoScript, ext:academic_partners
