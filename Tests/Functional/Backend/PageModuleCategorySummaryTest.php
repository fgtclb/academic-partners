...
namespace FGTCLB\AcademicPartners\Tests\Functional\Backend;
...
/**
 * The page module of a partner page shows the categories of that page above the content grid.
 *
 ...
 * module template.
 */
final class PageModuleCategorySummaryTest extends AbstractAcademicPartnersTestCase
{
    private const PARTNER_PAGE = 2;
    ...
    {
        $headerContent = $this->headerContentOf(self::PARTNER_PAGE);

        $this->assertStringContainsString('Europe', $headerContent);
        $this->assertStringContainsString('University', $headerContent);
    }

    /**
     * The second half of the defect this change repairs: the removed partial translated
     * `sys_category.academic_partners.{type}`, a key that exists in no XLF file of this
     * extension. The real keys are `sys_category.partners.*`, and the summary takes the
    ...
    public function everyCategoryTypeIsLabelledWithItsRegisteredTitle(): void
    {
        ...
        $this->assertStringContainsString('Region', $headerContent);
        $this->assertStringContainsString('Partner Type', $headerContent);
    }
    ...
    public function aTypeWithoutACategoryIsListedAsNotSet(): void
    {
        ...
        $this->assertStringContainsString('SDG', $headerContent);
        $this->assertStringContainsString('Not set', $headerContent);
    }

    /**
     * The fixture page carries the same two categories as the partner page, so an empty header
